...
const VideoCarousel = () => {
  ...
  const getLatestVideosByCategory = (): VideoItem[] => {
    ...
    categoriesWithoutAll.forEach(category => {
      ...
      if (categoryVideos.length > 0) {
        ...
        latestVideos.push(sortedVideos[0]);
      }
    });
  ...
  const carouselVideos = getLatestVideosByCategory();
      ...
      <div className="container mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Explore our latest tutorials from each category
          </p>
        </div>
          ...
          <div className="overflow-x-auto pb-6 hide-scrollbar">
            <div className="flex gap-6 min-w-max px-4">
              {carouselVideos.map((video) => (
                <div
                  key={video.id}
                  className="group bg-gray-50 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300 w-[380px] flex-shrink-0"
                >
